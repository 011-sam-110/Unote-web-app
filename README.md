...
<p align="center">A note-taking app for university students: lecture notes, flashcards and infinite boards in one place, with AI that only acts when you ask it to.</p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-877%20passing-2ea44f" alt="877 tests passing">
</p>
...
spaced schedule, and put the thinking that is not paragraphs on an infinite board. Bring in
what you already have: a PDF, a slide deck, photos of handwritten pages, a lecture recording,
or a Notion, Obsidian or Google Docs export. It runs in the browser, installs as a desktop
app, and keeps working with the network off.
...
- **No account needed at either end** - start writing straight from the landing page with no signup and keep the work when you make an account later; publish any note or board behind an unguessable link, optionally password-gated, view-only or editable, and revoke it whenever.

## 📸 Screenshots

| Ask across every note, with the sources cited | The assistant, scoped to the note you are reading |
|---|---|
| ![The Ask AI page answering a question with a table built from the user's own notes](docs/screenshots/ask.png) | ![The in-note assistant panel answering a question, with quick actions and an "add to note" control](docs/screenshots/assistant.png) |
| An infinite board, alongside the notes | Flashcards on an SM-2 schedule |
| ![A revision board of coloured stickies, a text note and a card linking to a real note](docs/screenshots/canvas.png) | ![The study page showing a flashcard question with Again, Hard, Good and Easy buttons](docs/screenshots/study.png) |
| Search operators, combining | The same editor in dark |
| ![A search for tag:exam returning three notes across three notebooks](docs/screenshots/search.png) | ![The note editor in dark theme](docs/screenshots/dark.png) |
...
```bash
npm run test -w server       # 568 unit + integration
npm run test -w web          # 309 unit
...
server/src/
  routes/             notes, search, study, canvas, share, sync, imports, ai, oauth
  ai/                 tool catalogue, per-caller key resolution, quota
  schema.sql          applied on boot
```

Four decisions that are not obvious from reading the code:
