 ...
 * write". `localApi` also carries deliberately EMPTY stubs - `versions`, `canvas`,
 * `ink`, `comments`, `templates` - so that a guest with no account gets an empty
 * panel instead of an error. Those stubs are the right answer for a guest and the
 * WRONG answer for a signed-in user online, whose real data is on the server. An
 * inferred list would quietly serve the stub and show them nothing.
 */
